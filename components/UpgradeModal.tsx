...
interface UpgradeModalProps {
  isOpen: boolean;
  onClose: () => void;
  // 🚀 新增：通过此参数区分是“主动升级”还是“额度耗尽”
  isExhausted?: boolean; 
}

export default function UpgradeModal({ isOpen, onClose, isExhausted = false }: UpgradeModalProps) {
          ...
          /* ==================================== */
            <>
              <div className="text-center">
                <div className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full ${isExhausted ? 'bg-red-50' : 'bg-blue-100'}`}>
                  {/* 🚀 动态判断图标：耗尽显示警告，主动升级显示星星 */}
                  <span className="text-2xl">{isExhausted ? "⚠️" : "✨"}</span>
                </div>
                {/* 🚀 动态判断标题 */}
                <h2 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">
                  {isExhausted ? "算力积分已耗尽" : "升级 PRO 获取更多算力"}
                </h2>
                {/* 🚀 动态判断副标题 */}
                <p className="mt-3 text-sm leading-7 text-gray-600 sm:text-base">
                  {isExhausted 
                    ? "您的可用积分已不足以完成本次操作。请购买积分包或升级 PRO 恢复创作。" 
                    : "解锁 PRO 会员或补充算力积分，享受无缝的创作心流。"}
                </p>
              </div>
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                ...
                <div className="relative flex flex-col rounded-[24px] border border-gray-200 bg-white p-6 shadow-sm transition-all hover:border-blue-500 hover:shadow-md">
                  <div className="mb-4 text-lg font-semibold text-gray-900">算力加油包</div>
                  <div className="mb-6 flex items-baseline text-3xl font-bold text-gray-900">
                    ¥9.9<span className="ml-1 text-sm font-medium text-gray-500">/ 1000 积分</span>
                  </div>
                  ...
                  <button 
                    // 注意：这里将 planType 修改为了 credit_1000 以区分旧逻辑
                    onClick={() => handleCreateOrder({ name: "1000 算力积分加油包", price: "9.9", planType: "credit_1000", amount: 990 })}
