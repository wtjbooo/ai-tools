      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div
              key={item.id}
              className="group relative bg-white rounded-[28px] p-6 border border-zinc-200/60 shadow-sm 
            ...
            >
              ...
              <div className="relative z-10 flex justify-between items-start mb-4">
                ...
                {/* 完美复刻的爱心按钮 (始终为红心，点击即取消) */}
                <button
                  onClick={() => handleRemoveFavorite(item.id)}
                  className="p-2 rounded-full bg-red-50 text-red-500 hover:bg-red-100 hover:scale-110 active:scale-90 transition-all"
                  title="取消收藏"
                >
