  ...
  return (
    <div className="space-y-6">
      ...
      <div className="min-h-[200px]">
        {/* 产品简介面板 */}
        {activeTab === "intro" && (
          <div className="space-y-5 text-[15px] leading-loose text-zinc-700 animate-in fade-in duration-500">
            {contentParagraphs.map((paragraph, index) => (
              <p key={`${toolId}-paragraph-${index}`}>{paragraph}</p>
            ))}
          </div>
        )}

        {/* 使用教程面板 (使用 ReactMarkdown 解析) */}
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
        <div className="space-y-8">
          ...
          <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr),320px]">
            <div className="space-y-8">
              {/* 核心改动：用全新的 TabsLayout 替换原来的 DetailCard */}
              <DetailCard
                title="产品介绍"
              >
                <TabsLayout 
